...
//
// Setup: a LightInfantry commander (base Range 3) on a peak platform vs a
// target commander 3.5 tiles away on low ground. Distance 3.5 > base range 3,
// so the shot connects ONLY via the flat +1 any-height-advantage bonus
// (effective 4). Half-tile margins on both sides of the boundary keep the
// test immune to spawn jitter (the session RNG is time-seeded in tests). The
// control run places the attacker on low ground at the same distance — no
// bonus, no hit.
func TestLiveHighGroundRangeBonus(t *testing.T) {
	...
	// run spawns an attacker commander + a target commander 3.5 tiles apart and
	...
	// on low ground (15,3); the target is always at (6.5,3) on low ground, so
	// the pair is 3.5 tiles apart in both runs.
	run := func(attackerHigh bool) (targetHP int32, found bool) {
		...
		if got := cs.ElevationFn(6, 3); got != 0 {
			t.Fatalf("ElevationFn(6,3)=%d, want 0 (low ground)", got)
		}
		...
		gs.SpawnSquadWithType(2, 2, fixed.FromFloat(6.5), ay, 0, component.UnitLightInfantry)
	...
	// Peak over low: +1 range → effective 4 ≥ distance 3.5 → target takes damage.
	hp, found := run(true)
	if !found {
	...
	// Control — both on low ground at distance 3.5: base range 3 < 3.5 → no hit.
	hp, found = run(false)
	if !found {
