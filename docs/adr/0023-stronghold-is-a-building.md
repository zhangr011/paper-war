...
- **Level-scaled damage split.** Incoming damage to a garrisoned stronghold is split garrison : stronghold (≈50% : 50% at L1 → 30% : 70% at L5); the garrison's share divides evenly across garrisoned units. Higher-level strongholds shelter their garrison better (garrison share drops, stronghold absorbs more). Garrisoned units can die from the split during a long siege. When the stronghold flips, surviving garrisoned units are evicted adjacent with their remaining HP. (Reversed from an earlier draft — `StrongholdGarrisonShare` L1=50→L5=30.)
