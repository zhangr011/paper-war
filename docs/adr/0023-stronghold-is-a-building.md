...
- **Level-scaled damage split.** Incoming damage to a garrisoned stronghold is split stronghold : garrison (≈30% : 70% at L1 → 50% : 50% at L5); the garrison's share divides evenly across garrisoned units. The garrison absorbs the majority — a stronghold is a firing/buff platform, not a safe room. Garrisoned units can die from the split during a long siege. When the stronghold flips, surviving garrisoned units are evicted adjacent with their remaining HP.
