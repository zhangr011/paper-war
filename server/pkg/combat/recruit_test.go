...
func TestRecruitWeaponSlotMismatch(t *testing.T) {
	w, _, recruitSys, _, unitTypePool, _, _, cmd := setupRecruitWorld()

	// Commander has Light weapons (Gun), try to recruit Heavy unit (Cannon)
	recruitSys.Recruit(RecruitRequest{
		CommanderEntity: cmd,
		UnitType:        component.UnitHeavyInfantry, // Cannon = Heavy weapon
	})

	w.Tick(1)

	// Should NOT have created a new entity (weapon slot mismatch)
	newEntity := ecs.Entity(2)
	if _, ok := unitTypePool.Get(newEntity); ok {
		t.Error("should not recruit Heavy unit with Light weapon commander")
	}
}
...
func TestRecruitSquadSizeLimit(t *testing.T) {
	...
	// Recruit 12 units (should fill up at max squad size)
	for i := 0; i < 12; i++ {
		recruitSys.Recruit(RecruitRequest{
			CommanderEntity: cmd,
			UnitType:        component.UnitLightInfantry,
