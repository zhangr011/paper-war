...
import { UNIT_RANGES, effectiveRange, elevationAt, ringTriangles } from './range_ring.js?v=v9';
...
export class Game {
  constructor(existingConnection) {
    ...
    this._rangeRingPool = [];       // buildRangeRings (ring descriptor objects)
  }
  ...
  render() {
    ...
    // Build selection highlight descriptors + range rings
    const selectionHighlights = this.buildSelectionHighlights();
    const rangeRings = this.buildRangeRings(allUnits);
    ...
    // Pass 4.1: Range rings (terrain-readability Phase 1) — base range +
    // high-ground envelope for selected units. Same effects batch.
    if (rangeRings.length > 0) {
      this.renderer.drawRings(rangeRings, cameraOffset);
    }
  ...
  buildUnitDescriptors(units) {
    ...
    for (const unit of units) {
      ...
      // Commanders render as a HEAVIER atlas type — a totally different sprite
      // cycle (idle/move/attack/die all use different frames + silhouette).
      // Each type maps one tier up so the commander reads as bigger/imposing.
      const CMD_ATLAS_TYPE = [1, 4, 5, 6, 5, 6, 6];
      const atlasType = unit.isCommander ? (CMD_ATLAS_TYPE[unitType] || unitType) : unitType;
      const cell = atlasCell(atlasType, state, dir, frame);
      ...
      // Commander: static gold tint on the heavier sprite (#54).
      if (unit.isCommander) {
  ...
  /**
   * Build range-ring descriptors for selected units (terrain-readability
   * Phase 1 / ADR-0029): a ring at base attack range, plus an outer ring at
   * base + attackerElevation when the unit stands on raised ground — the
   * high-ground bonus made visible. Flat ground draws one ring only.
   * Ring geometry is generated by range_ring.ringTriangles; the pool array
   * is reused frame to frame.
   */
  buildRangeRings(allUnits) {
    const rings = this._rangeRingPool;
    rings.length = 0;
    if (!this.elevationData) return rings;
    const zoom = this.camera.zoom;
    const tilePx = TILE_WIDTH * zoom;

    for (const unit of this.selectedUnits) {
      if (!unit.alive) continue;
      const elev = elevationAt(
        this.elevationData, this.mapWidth, this.mapHeight,
        unit.renderX, unit.renderY,
      );
      // Ramp check (ADR-0034): a unit standing on a Ramp never gains the
      // bonus, whatever the Ramp's authored elevation.
      const tx = Math.floor(unit.renderX), ty = Math.floor(unit.renderY);
      const onRamp = tx >= 0 && ty >= 0 && tx < this.mapWidth && ty < this.mapHeight
        && this.terrainData && this.terrainData[ty * this.mapWidth + tx] === 18; // TerrainRamp

      // Per-target bonus (ADR-0029/0034): the +1 exists only against a LOWER
      // target, so the outer ring appears only when an enemy within max
      // reach actually stands lower than this unit.
      const scanR = (UNIT_RANGES[unit.unitType] ?? 3) + 1;
      const enemies = [];
      for (const other of allUnits) {
        if (!other.alive || other.team === unit.team) continue;
        const dx = other.renderX - unit.renderX;
        const dy = other.renderY - unit.renderY;
        if (dx * dx + dy * dy > scanR * scanR) continue;
        enemies.push({ elev: elevationAt(
          this.elevationData, this.mapWidth, this.mapHeight,
          other.renderX, other.renderY,
        ) });
      }
      const { base, max } = effectiveRange(unit.unitType, elev, { onRamp, enemies });
      // renderX/renderY IS the unit's world position (tiles) — the sprite
      // grows outward from it — so the ring centers exactly there.
      const cx = unit.renderX * tilePx;
      const cy = unit.renderY * tilePx;
      const thickness = Math.max(2, tilePx * 0.06);
      const segments = 48;

      // Base ring (solid, team-tinted).
      const verts = [];
      ringTriangles(cx, cy, base * tilePx, thickness, segments, verts);
      rings.push({ verts, r: SELECTION_COLOR.r, g: SELECTION_COLOR.g, b: SELECTION_COLOR.b, a: 0.5 });

      // High-ground envelope (fainter) — only when it actually extends.
      if (max > base) {
        const outer = [];
        ringTriangles(cx, cy, max * tilePx, thickness, segments, outer);
        rings.push({ verts: outer, r: 0.95, g: 0.78, b: 0.2, a: 0.45 }); // gold — bonus range
      }
    }
